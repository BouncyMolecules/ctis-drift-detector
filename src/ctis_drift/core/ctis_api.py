"""Client for the official CTIS **public** API at `euclinicaltrials.eu`.

Endpoints (documented externally and observed in browser traffic):

- ``POST https://euclinicaltrials.eu/ctis-public-api/search`` — paginated summaries
- ``GET https://euclinicaltrials.eu/ctis-public-api/retrieve/{euct_number}`` — full record

Design goals: session reuse, bounded retries with exponential backoff,
typed Pydantic models, Streamlit-aware response caching via ``st.cache_data``,
and respectful per-request pacing (small delay configurable).
"""
...
class CtisPublicClient:
    ...
    def _request_json(
        ...
        *,
        ...
        try:
            ...
            preview = _json_preview(response.text or "")
            logger.exception("CTIS JSON decode failed (%s)", normalized)
    ...
    def search_trials(
        ...
        try:
            ...
            logger.exception("TrialSearchResponse validation failed")
    ...
    def get_full_trial(self, euct_number: str) -> TrialFullRecord:
        ...
        try:
            ...
            logger.exception("TrialFullRecord validation failed for %s", sanitized)
            preview = _json_preview(json.dumps(raw))
            raise CtisMalformedJsonError(
...
def public_client_singleton(
    *,
    ...
    """Shared process-wide synchronous client reused by cached helpers."""
...
def _search_trials_cached(payload_json: str) -> TrialSearchResponse:
    """Streamlit-backed cache for search payloads (JSON-stable key)."""
...
@st.cache_data(ttl=DEFAULT_CACHE_TTL_SECONDS, show_spinner=False)
def _get_full_trial_cached(euct_number: str) -> TrialFullRecord:
    """Streamlit-backed cache keyed by sanitized EUCT number."""
...
def search_trials(
    *,
    ...
    """High-level POST ``search`` helper with Streamlit memoization enabled by default.

    When ``use_streamlit_cache`` is enabled, identical payloads reuse responses for
    ``DEFAULT_CACHE_TTL_SECONDS``. Disable for interactive scripts or tests needing
    freshest data on every invocation.

    Arguments:
        payload: Optional overrides for pagination/sorting/filters.

    Returns:
        :class:`TrialSearchResponse`.

    Raises:
        CtisPublicApiError: propagated from :class:`CtisPublicClient`.
    """

    body = payload or TrialSearchPayload()
    payload_json = body.model_dump_json(by_alias=True, exclude_none=True)
    if use_streamlit_cache:
        logger.debug("Using Streamlit cached search_trials(payload_hash=stable_json)")
        return _search_trials_cached(payload_json)
...
def get_full_trial(
    ...
    *,
    ...
    """High-level retrieve helper respecting Streamlit caches by trial id."""

    sanitized = euct_number.strip().strip("/")
    if use_streamlit_cache:
        return _get_full_trial_cached(sanitized)
...
class CTISAPIClient(CtisPublicClient):
    ...
    Prefer calling :func:`search_trials` / :func:`get_full_trial` in new codepaths.
    """
